...
object SocketTextStreamWordCount {
    ...
    def main(args : Array[String]) {
        ...
        if ( args.length != 3 ) {
            System.err.println("USAGE:\nSocketTextStreamWordCount <job_name> <hostname> <port>")
            return
        }
        ...
        val counts = text.flatMap { _.toLowerCase.split("\\W+") filter { _.nonEmpty } }
            .map(new SimpleMap).name("countSet").uid(s"${name}_operator_countSet")
            .keyBy(0)
            .sum(1)
    ...
    class SimpleMap extends RichMapFunction[String, (String, Int)] {

        var meter: org.apache.flink.metrics.Meter = null
